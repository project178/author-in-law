...
with open("dataset6", "w", encoding="utf8") as outp:
    writer = writer(outp, delimiter="\t")
    writer.writerow(["text", "author"])
    for page in range(13):
        soup = BeautifulSoup(requests.get("http://istmat.info/documents?tid_theme=All&tid_area=All&tid_state=All&tid_type=All&tid_tags=&period=&title=%D0%B4%D0%BE%D0%BF%D1%80%D0%BE%D1%81&page=" + str(page)).text, 'lxml')
