...
authors_and_texts = {}
with open("data/dataset1.csv", encoding="utf8") as inp:
...
with open("data/text", "wb") as text_corp: dump(texts, text_corp)

with open("data/text", "rb") as tmp: texts = load(tmp)
...
texts = [[sub("(\W+\s)|(\W+$)", lambda m: " "+str(m[0]), part) for part in text] for text in texts]
texts = [[sub("[\[\]]", "", part) for part in text] for text in texts]
elmo = ElmoEmbedder()
elmo_corp = [[elmo.embed_sentence(part.split()) for part in text] for text in texts]
with open("data/elmo", "wb") as elmo_out: dump(elmo_corp, elmo_out)
...
with open("stop") as stop:
    for line in stop:
        if line.strip() not in stopwords: stopwords.add(line)
...
with open("data/tfidf", "wb") as tmp: dump(tfidf_corp, tmp)
with open("data/tfidf_params", "wb") as tmp: dump(vectorizer.get_params(), tmp)
predictor = RNNMorphPredictor(language="ru")
